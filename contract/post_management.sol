...
library Util {
    function generateRandomId(address addr, string calldata _name) internal view returns (bytes32) {
        bytes32 hash = keccak256(abi.encodePacked(block.timestamp, addr, _name));
        return hash;
    }
}
...
contract PostManagement {
    ...
    function CreatePost(string calldata _name, string calldata addr, string calldata website, string memory description) external {
        // 发起帖子
        require(AllPostOwner[msg.sender] == 0, "you have a post");
        string memory desp = bytes(description).length != 0 ? description : "This post has no description yet.";
        
        bytes32 post_id = Util.generateRandomId(msg.sender, _name);
        PostInfo memory post = PostInfo(_name, addr, website, msg.sender, desp, block.timestamp, post_id);
    ...
    function UpdataPost(bytes32 post_id, string calldata _name, string calldata addr, string calldata website, string calldata desp, address owner) external{
        // 修改帖子信息
...
contract CommentManagement {
    struct Comment {
        ...
        uint comment_time;  // 评论时间
    }

    mapping (bytes32 => Comment[]) public TotalComments;  // 所有评论[bytes32对应帖子的id]

    function post_comment(bytes32 post_id, string calldata text) external {
        // 发布评论
        TotalComments[post_id].push(Comment(msg.sender, text, block.timestamp));
    }

    function get_comment_length(bytes32 post_id) external view returns(uint){
        
        return TotalComments[post_id].length;
    }
}
...
contract RelationshipManagment {
    ...
    function invite_cooperation(string memory post_managment_addr, bytes32 postid) external {
        address myAddress = address(bytes20(bytes(post_managment_addr)));
        // 邀请合作
        bytes32 inviter_postid = PostManagement(myAddress).getPostIdByOwner(msg.sender);
        inviter_map[postid].push(cooperate(inviter_postid, 0));
        invitee_map[inviter_postid].push(cooperate(postid, 0));
    }

    function set_invitation_status(address post_managment_addr, bytes32 postid, uint _status) external {
        if (_status == 1 || _status == 2){
            bytes32 self_postid = PostManagement(post_managment_addr).getPostIdByOwner(msg.sender);
